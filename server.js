...
axios
        ...
        .then(pokemon_response  =>{
            const myPokemonsLinks = [];
            for(i=0; i<250; i++){
                axios
                .get(pokemon_response.data.results[i].url)
                .then(link_response => {
                    //Format types names of pokemon
                    let typesNames = []
                    link_response.data.types.forEach((typeData)=>{
                        typesNames.push(typeData.type.name);
                    })
                    let id = link_response.data.id;
                    ...
                    card.name = link_response.data.name,
                    card.id = id,
                    card.weight = link_response.data.weight,
                    card.height = link_response.data.height,
                    card.base_experience = link_response.data.base_experience,
...
//Random number by range function 
...
app.get('/get5', async (req, res) => {
    ...
    while(cards.length<5){
        randomId = between(0,251);
        card = await pokemonCardDB.find({ id : randomId, active : false})
        ...
        await pokemonCardDB.updateOne({id : randomId}, { $set:{active : true}})
        card[0].active = true;
        cards.push(card);
    } 
    res.send(cards)
...
app.get('/isActive/:idGame', async (req, res) => {
    let idGame = req.params.idGame;
    //Get active games
    await pokemonCardDB.find({active : true})
    res.send(true)
});

app.get('/getActive', async (req, res) => {
    //Get active cards
    res.send(await pokemonCardDB.find({active : true}))
});

app.get('/setInactive/', async (req, res) => {
    await pokemonCardDB.updateMany({}, { $set:{active : false}})
    //Get active cards
    res.send()
});


app.get('/setInactive/:namePokemon', async (req, res) => {
    let namePokemon = req.params.namePokemon
    await pokemonCardDB.updateOne({name : namePokemon}, { $set:{active : false}})
    //Get active cards
    res.send()
});
...
app.post('/createCard', async (req, res) => {
    ...
    //Search in database
    let data=[]
    data = await pokemonCardDB.find({typeCard: type_card, name: name_card});
    console.log(data);
    if(!data){
        console.log("Card already in database");
        res.send("Card already in database")
    }
    else{
